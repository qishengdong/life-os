...
export function isValidInviteCodeFormat(code: string): boolean {
  return /^LO-[A-Z2-9]{4}-[A-Z2-9]{4}$/.test(code);
}
...
export function normalizeInviteCode(input: string): string {
  ...
  // 如果用户漏了横线, 自动补 (10 位 → LO-XXXX-XXXX)
  if (/^LO[A-Z2-9]{8}$/.test(cleaned)) {
    return `LO-${cleaned.slice(2, 6)}-${cleaned.slice(6, 10)}`;
  }
  return cleaned;
}

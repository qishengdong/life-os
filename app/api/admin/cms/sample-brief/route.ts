...
import {
  loadSampleBriefContent,
  writeSampleBriefContent,
  validateSampleBriefContent,
} from '@/lib/content/sample-brief';
...
export async function GET(req: NextRequest) {
  ...
  try {
    return NextResponse.json({ content: loadSampleBriefContent() });
  } catch (e: any) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      writeSampleBriefContent(content);
      localSaved = true;
    } catch {}
    ...
    if (publish) {
      ...
      try {
        publishResult = await commitFileToGitHub({
          path: 'lib/content/data/sample-brief.json',
          content: JSON.stringify(content, null, 2) + '\n',
          message: `cms(sample-brief): 更新内容 (by ${user.displayName} · ${user.username})`,

...
import {
  ...
  writeMembershipContent,
  validateMembershipContent,
...
export async function GET(req: NextRequest) {
  ...
  try {
    return NextResponse.json({ content: loadMembershipContent() });
  } catch (e: any) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      writeMembershipContent(content);
      localSaved = true;
    } catch {}
    ...
    if (publish) {
      ...
      try {
        publishResult = await commitFileToGitHub({
          path: 'lib/content/data/membership.json',
          content: JSON.stringify(content, null, 2) + '\n',
          message: `cms(membership): 更新内容 (by ${user.displayName} · ${user.username})`,

...
export async function GET(req: NextRequest) {
  try {
    ...
    const commitments = dueOnly ? getDueCommitments(userId) : getUserCommitments(userId);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const all = getUserCommitments(userId);
    ...
    if (action === 'fulfill') {
      markFulfilled(commitmentId);
    } else if (action === 'cancel') {
      markCancelled(commitmentId);
    } else {
